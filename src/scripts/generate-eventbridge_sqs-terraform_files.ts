...
async function main(): Promise<void> {
  const eventBusName = 'cronos.domain_events'
...
function generateSQSQueueTerraform(queue: QueueConfig): string {
  ...
  return `
    data "aws_iam_policy_document" "policy_${queue.name.replaceAll('.', '-')}" {
      statement {
        ...
        resources = [
          aws_sqs_queue.${queue.name}.arn,
          aws_sqs_queue.${deadLetterQueueName}.arn,
        ]
      }
    }
    ...
    resource "aws_sqs_queue_policy" "sqs_policy_${queue.name.replaceAll('.', '-')}" {
      queue_url = aws_sqs_queue.${queue.name}.id
      policy    = data.aws_iam_policy_document.policy_${queue.name.replaceAll('.', '-')}.json
    }

    resource "aws_sqs_queue_policy" "sqs_policy_${deadLetterQueueName.replaceAll('.', '-')}" {
      queue_url = aws_sqs_queue.${deadLetterQueueName}.id
      policy    = data.aws_iam_policy_document.policy_${queue.name.replaceAll('.', '-')}.json
    }
`
}
